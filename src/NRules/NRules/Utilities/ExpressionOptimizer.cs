...
internal static class ExpressionOptimizer
{
    ...
    public static Expression<TDelegate> Optimize<TDelegate>(LambdaExpression expression,
        IndexMap factIndexMap, IEnumerable<DependencyElement> dependencies, IndexMap dependencyIndexMap)
    {
    ...
    private static void ResolveDependencies(ExpressionParts parts,
        IEnumerable<DependencyElement> dependencies, IndexMap indexMap)
    {
        ...
        var pairs = dependencies.Zip(indexMap, (d, i) => (d, i));
        foreach (var (d, i) in pairs)
        {
            if (i < 0)
                continue;

            var parameter = parts.BodyParameters[i];
            var parameterType = d.ServiceType;
