...
public class DgmlWriter
{
    ...
    private DirectedGraph CreateGraph()
    {
        ...
        var reteLinks = FilterLinks(reteNodes, _schema.Links);
    ...
    private void AddPerformanceMetrics(DirectedGraph graph)
    {
        ...
        if (_metricsProvider is not null)
        {
            ...
            foreach (var node in graph.Nodes)
            {
                ...
                var nodeMetrics = _metricsProvider.FindByNodeId(reteNode.Id);
                if (nodeMetrics == null)
                    continue;
    ...
    private static void AddPerformanceMetrics(Node node, INodeMetrics nodeMetrics)
    {
        ...
        node.Properties.Add("PerfTotalInputCount", nodeMetrics.InsertInputCount + nodeMetrics.UpdateInputCount + nodeMetrics.RetractInputCount);
        node.Properties.Add("PerfTotalOutputCount", nodeMetrics.InsertOutputCount + nodeMetrics.UpdateOutputCount + nodeMetrics.RetractOutputCount);
        node.Properties.Add("PerfTotalDurationMilliseconds", nodeMetrics.InsertDurationMilliseconds + nodeMetrics.UpdateDurationMilliseconds + nodeMetrics.RetractDurationMilliseconds);
    }
    ...
    private static IEnumerable<ReteLink> FilterLinks(IEnumerable<ReteNode> reteNodes, IEnumerable<ReteLink> reteLinks)
    {
