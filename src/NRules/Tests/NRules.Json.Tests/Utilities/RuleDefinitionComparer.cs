...
public static class RuleDefinitionComparer
{
    public static bool AreEqual(IRuleDefinition? x, IRuleDefinition? y)
    {
        ...
        if (x.Priority != y.Priority)
            return false;
        if (x.Repeatability != y.Repeatability)
            return false;

        if (!AreEqual(x.DependencyGroup, y.DependencyGroup))
            return false;
        if (!AreEqual(x.LeftHandSide, y.LeftHandSide))
            return false;
        if (!AreEqual(x.FilterGroup, y.FilterGroup))
            return false;
        if (!AreEqual(x.RightHandSide, y.RightHandSide))
            return false;
