...
public class AggregateBuilder : RuleElementBuilder, IBuilder<AggregateElement>
{
    ...
    AggregateElement IBuilder<AggregateElement>.Build()
    {
        ...
        {
            throw new InvalidOperationException("Source was not specified for Aggregate element");
        }
        if (_resultType is null)
        {
            throw new InvalidOperationException("Result type was not specified for Aggregate element");
        }
        if (_name is null)
        {
            throw new InvalidOperationException("Name was not specified for Aggregate element");
        }

        var sourceElement = _sourceBuilder.Build();
        return Element.Aggregate(_resultType, _name, _expressions, sourceElement, _customFactoryType);
    }
