...
namespace NRules.Core.IntegrationTests.Tests.Helpers
{
    internal class DependencyFactory
    {
        public static IContainer GetContainer(EventHandler handler)
        {
            ...
            container.Stub(x => x.GetObjectInstance(typeof (SimplePersonalFinancesRule))).Return(
                new SimplePersonalFinancesRule(handler));
            container.Stub(x => x.GetObjectInstance(typeof (SelfBeneficialPolicyRule))).Return(
                new SelfBeneficialPolicyRule(handler));
            return container;
        }
    }
}